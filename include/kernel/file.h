#ifndef __DDMOS_FILE_H__
#define __DDMOS_FILE_H__
...
//
uint8t *check_filename(uint8t *filename, uint16t filename_length);
...
uint16t delete_file(uint8t *filename, uint16t filename_length)
{
    uint8t drive_num       = multiboot_ptr->boot_device;  // Get drive #
    uint16t error_code     = 0;
    uint8t file_size       = 0;
    uint8t starting_sector = 0;
    uint8t ft_size         = 0;
    uint8t ft_start_sector = 0;
    uint8t *free_space     = "FREE SPACE";
    uint8t *filetable      = "fileTable ";
    uint8t *file_ptr;
    uint8t test_byte;
    ...
    file_ptr = (uint8t *)0x20000;

	// Write nulls to memory location
    for (uint16t i = 0; i < file_size*512; i++) 
        file_ptr[i] = 0;
...
//       4: File extension
// output 1: Return code
uint16t load_file(uint8t *filename, uint16t filename_length, uint32t address, uint8t *file_ext)
{
    uint8t drive_num       = *(uint8t *)0x1500;  // Get drive #
    uint16t error_code     = 0;
    uint16t return_code    = 0;
    uint8t file_size       = 0;
    uint8t starting_sector = 0;
    uint8t *file_ptr;
    uint8t test_byte;
...
// Output 1: Return code
uint16t rename_file(uint8t *file1, uint16t file1_length, uint8t *file2, uint16t file2_length)
{
    uint8t drive_num       = *(uint8t *)0x1500;  // Get drive #
    uint16t error_code     = 0;
    uint16t return_code    = 0;
    ...
    for (uint8t i = 0; i < file2_length; i++)
        file_ptr[i] = file2[i];
...
// Output 1: Return code
uint16t save_file(uint8t *filename, uint8t *file_ext, uint8t in_file_size, uint32t address)
{
    uint8t drive_num         = *(uint8t *)0x1500;  // Get drive #
    uint16t error_code       = 0;
    uint16t return_code      = 0;
    uint16t filename_length  = 10;
    uint8t file_size         = 0;
    uint8t starting_sector   = 0;
    uint8t ft_size           = 0;
    uint8t ft_start_sector   = 0;
    uint8t last_saved_sector = 0;
    uint8t *free_space       = "FREE SPACE";
    uint8t *filetable        = "fileTable ";
    uint8t *file_ptr;
    uint8t test_byte;
...
{
    uint16t i; 
    uint8t *file_ptr = (uint8t *)0x1000;    // 1000h = location of fileTable in memory
